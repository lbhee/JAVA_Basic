...
// 배열(Array)
// 배열이란 지정된 크기만큼의 똑같은 데이터타입이 모여있는 하나의 자료형.
...
// 예를 들어 우리가 int 배열 intAarry의 2번째 값을 할당할 때에는
// intArray[2] = 30; 이런식으로 값을 할당하거나
// 값을 호출할 때에는
...
// null이란 특수한 "상태"로써, 
// 참조형 데이터타입이 메모리 주소값이 부여되었지만
// 해당 메모리 주소값을 참조해서 들어가면 아무것도 없는 상태이다.
// null인 상태에서는 새로운 값을 할당하는 것 외에는 아무것도 할 수 없다.

// 또한 배열의 존재하지 않는 위치를 접근하려고 한다면
// ArrayIndexOutOfBoundsException
// 즉 잘못된 위치 접근 에러가 발생한다.
// 위치를 지정하는 위치번호"는 우리가 index라고 부른다.
// 배열의 인덱스는 0부터 (해당배열크기 - 1) 까지이다.
// Q : 크리가 5인 배열의 인덱스 범위는?
// A : 0 1 2 3 4

public class Ex01Array1 {
